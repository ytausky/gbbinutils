...
mod tests {
    ...
    fn analyze_ld_deref_symbol_a() {
        ...
        assert_eq!(
            analyze(
                Command::Ld,
                vec![
                    SynExpr::from(Token::Atom(Atom::Ident(ident))).deref(),
                    atom(A),
                ]
            ),
    ...
    fn describe_ld_reg16_immediate(dest: Reg16) -> InstructionDescriptor {
        ...
        (
            (
                ...
                vec![
                    SynExpr::from(dest),
                    SynExpr::from(Token::Atom(Atom::Ident(value))),
                ],
            ),
    ...
    fn describe_branch(branch: BranchKind, condition: Option<Condition>) -> InstructionDescriptor {
        ...
        operands.push(SynExpr::Atom(Token::Atom(Atom::Ident(ident))));
        (
